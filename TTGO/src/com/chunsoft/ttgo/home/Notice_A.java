...
public class Notice_A extends Activity implements IXListViewListener {
	@Bind(R.id.x_lv)
	XListView x_lv;
	@Bind(R.id.tv_title)
	TextView tv_title;
	...
	private void init() {
		...
		GetNoticeData(rows, page, new VolleyDataCallback<NoticeBean>() {
			...
			public void onSuccess(NoticeBean datas) {
				page++;
				adapter = new NoticeAdapter(Notice_A.this, datas.rows,
	...
	private void GetNoticeData(int rows, int page,
		...
		if (dialog == null) {
			dialog = ProgressDialog.show(Notice_A.this, "", "正在加载...");
			dialog.show();
		}
		...
		GsonRequest<NoticeBean> req = new GsonRequest<NoticeBean>(Method.GET,
				...
				}, new AbstractVolleyErrorListener(Notice_A.this) {
					@Override
					public void onError() {
						if (dialog != null && dialog.isShowing()) {
							dialog.dismiss();
