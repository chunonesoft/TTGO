...
import com.chunsoft.view.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // 得到Header的高度，这个高度�?��用这种方式得到，在onLayout方法里面得到的高度始终是0
    ...
    private void refreshLoadingViewsSize() {
        // 得到header和footer的内容高度，它将会作为拖动刷新的�?��临界值，如果拖动距离大于这个高度
        // 然后再松�?��，就会触发刷新操�?        
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            // 3，isPullLoading()，理由与�?条相�?            
            if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading())  {
                mLastMotionY = event.getY();
                // 第一个显示出来，Header已经显示或拉�?                
                if (isPullRefreshEnabled() && isReadyForPullDown()) {
                    // 1，Math.abs(getScrollY()) > 0：表示当前滑动的偏移量的绝对值大�?，表示当前HeaderView滑出来了或完�?                   
                	// 不可见，存在这样�?��case，当正在刷新时并且RefreshableView已经滑到顶部，向上滑动，那么我们期望的结果是
                    // 依然能向上滑动，直到HeaderView完全不可�?                    
                	// 2，deltaY > 0.5f：表示下拉的值大�?.5f
    ...
    public void onPullDownRefreshComplete() {
        if (isPullRefreshing()) {
            ...
            // 回滚动有�?��时间，我们在回滚完成后再设置状�?为normal
            // 在将LoadingLayout的状态设置为normal之前，我们应该禁�?            // 截断Touch事件，因为设里有�?��post状�?，如果有post的Runnable
    ...
    /**
     * �?��刷新，�?常用于调用�?主动刷新，典型的情况是进入界面，�?��主动刷新，这个刷新并不是由用户拉动引起的
     * 
     * @param smoothScroll 表示是否有平滑滚动，true表示平滑滚动，false表示无平滑滚�?     * @param delayMillis 延迟时间
     */
    ...
    protected void addRefreshableView(Context context, T refreshableView) {
        ...
        // 这里把Refresh view的高度设置为�?��很小的�?，它的高度最终会在onSizeChanged()方法中设置为MATCH_PARENT
        // 这样做的原因是，如果此是它的height是MATCH_PARENT，那么footer得到的高度就�?，所以，我们先设置高度很�?        // 我们就可以得到header和footer的正常高度，当onSizeChanged后，Refresh view的高度又会变为正常�?
        height = 10;
    ...
    protected void startRefreshing() {
        ...
        if (null != mRefreshListener) {
            // 因为滚动回原始位置的时间�?00，我们需要等回滚完后才执行刷新回�?            
        	postDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    protected void startLoading() {
        ...
        if (null != mRefreshListener) {
            // 因为滚动回原始位置的时间�?00，我们需要等回滚完后才执行加载回�?            
        	postDelayed(new Runnable() {
                @Override
                public void run() {
